...
FHIR.oauth2.ready().then(function(client) {
  ...
  var query_med = new URLSearchParams();
  query_med.set("patient", client.patient.id);

  client.request('MedicationRequest?' + query_med, {
    pageLimit: 0,
    flat: true
  }).then(
      function(medResults) {
        console.log("Med results", medResults);
        medResults.forEach(function(med) {
          displayMedication(med.medicationCodeableConcept.text);
          console.log("text", med.medicationCodeableConcept.text);
        })
      });
